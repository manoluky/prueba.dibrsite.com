...
const { validarFormulario } = require('../main/validate.js');
...
describe('Validación de formulario', () => {
  ...
  it('debería devolver un objeto con la propiedad "valido" igual a false si el campo Teléfono no tiene un formato válido', () => {
    const data = {
      nombre: 'John Doe',
      email: 'john@example.com',
      telefono: 'abc12345',
    ...
    assert.strictEqual(resultado.valido, false);
    assert.strictEqual(resultado.errores.length, 1);
    assert.ok(resultado.errores.includes('El campo Teléfono debe ser un número que comience con '));
  });

  it('debería devolver un objeto con la propiedad "valido" igual a false si se selecciona "contacto por teléfono" pero no se especifica la fecha y hora', () => {
    const data = {
      nombre:  'John Doe',
      email: 'john@example.com',
      telefono: '12345678',
    ...
    assert.strictEqual(resultado.valido, false);
    assert.strictEqual(resultado.errores.length, 1);
    assert.ok(resultado.errores.includes('Debes seleccionar una fecha y hora para ser contactado por teléfono.'));
  });

  it('debería devolver un objeto con la propiedad "valido" igual a false si se selecciona una opción inválida en el campo "Vende o Compra"', () => {
  const data = {
  nombre: 'John Doe',
  email: 'john@example.com',
  telefono: '12345678',
  mensaje: 'Este es un mensaje de prueba.',
  opciones: 'Opción inválida',
  contacto: 'correo',
  fecha: '2023-03-16',
  hora: '09:00'
  };const resultado = validarFormulario(data);
